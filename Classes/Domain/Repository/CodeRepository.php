...
namespace F3\Converter\Domain\Repository;
...
class CodeRepository extends \F3\FLOW3\Persistence\Repository {
	...
	/**
	 * @param \F3\Converter\Domain\Model\Codetype $ct
	 * @return int
	 */
	public function findCountByCodetype(\F3\Converter\Domain\Model\Codetype $ct) {
		$query = $this->createQuery();
	...
	/**
	 * @param \F3\Converter\Domain\Model\Codetype $ct
	 * @return \F3\Converter\Domain\Model\CodeRepository
	 */
	public function findByCodetype(\F3\Converter\Domain\Model\Codetype $ct) {
		$query = $this->createQuery();
		$result = $query->matching($query->equals('codetype', $ct))
				->setOrderings(array('modified' => \F3\FLOW3\Persistence\QueryInterface::ORDER_DESCENDING))
				->execute();
